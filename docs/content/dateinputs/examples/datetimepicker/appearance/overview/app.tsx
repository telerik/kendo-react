...
const App = () => {
    const config = useConfigurator({
        ...
        fillMode: DateTimePickerProps['fillMode'];
        rounded: Exclude<DateTimePickerProps['rounded'], 'none'>;
    };

    return (
        <div style={{ display: 'flex', justifyContent: 'center' }}>
            <DateTimePicker size={config.size} fillMode={config.fillMode} rounded={config.rounded} width={300} />
        </div>
    );
};
