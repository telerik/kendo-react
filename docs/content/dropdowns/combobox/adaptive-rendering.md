...
To set the title of the dialog window rendered when the ComboBox is opened on a smaller screen, set the [`adaptiveTitle`]({% slug api_dropdowns_comboboxprops %}) property of the component.
...
The following example demonstrates how the ComboBox adapts its popup element rendering to the screen size based on the configured breakpoints.
