...
To install the Gauges package, run the following command in the root folder of your React project:
...
Now try to add another component from the Gauges package yourself. The procedures for [installing](#install-the-component), [importing](#importing-the-components), and [using](#using-the-components) the Gauges components are identical for all components in the package.
