...
import { generateData } from './financial-data';

export const updateRandomPrices = (data: any[]) => {
...
export const updateAllPrices = (data: any[]) => {
    const newData = data.slice();
...
export const randomizeObjectData = (dataObj: any) => {
...
export const DataProvider = (props: any) => {
    ...
    const onDataReset = React.useCallback((volume = 10) => {
        ...
        setData(generateData(volume));
    }, []);

    const onStartLiveUpdate = React.useCallback((interval: number) => {
        clearInterval(intervalRef.current!);
