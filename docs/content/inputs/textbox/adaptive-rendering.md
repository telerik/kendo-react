...
description: 'Configure the on-screen keyboard for the KendoReact TextBox to implement adaptiveness in your React projects.'
components: ["textbox"]
...
To enhance the mobile user experience of your React apps, you can configure the type of the on-screen keyboard for the KendoReact TextBox. Use the [`inputAttributes`]({% slug api_inputs_textboxprops %}#toc-inputAttributes) property to set the `inputMode`. Based on the `inputMode` setting, the browser will display the most appropriate virtual keyboard.
...
{% meta height:300 %}
{% embed_file textbox/adaptive-rendering/app.tsx preview %}
{% embed_file textbox/adaptive-rendering/main.tsx %}
{% endmeta %}
