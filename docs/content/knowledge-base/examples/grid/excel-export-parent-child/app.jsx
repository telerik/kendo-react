            ...
            .then(() => {
                new Promise((resolve, reject) => {
                    for (let idx = data.length - 1; idx >= 0; idx--) {
                        let products = allProducts.filter((element) => element.CategoryID === data[idx].CategoryID);
                        for (let productIdx = products.length - 1; productIdx >= 0; productIdx--) {
                            const product = products[productIdx];
                            rows.splice(idx + 2, 0, {
                                cells: [{}, { value: product.ProductID }, { value: product.ProductName }]
                            });
                        }
                        rows.splice(idx + 2, 0, {
                            cells: [
    ...
    return (
        <div>
            <ExcelExport data={categories} ref={exportRef}>
                ...
                >
                    <GridToolbar>
                        <Button type="button" title="Export PDF" themeColor="primary" onClick={exportToExcel}>
                            Export to Excel
                        </Button>
                    </GridToolbar>
