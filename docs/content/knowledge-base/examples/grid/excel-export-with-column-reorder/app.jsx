...
const App = () => {
	...
	const [result, setResult] = React.useState(processWithGroups(products, initialDataState));
	...
	const getCells = (columns, cellProps) => {
		...
		columns.forEach((column) => {
			if (column.aggregate) {
				cells.push(<td>{cellProps.dataItem.aggregates[column.field][column.aggregate]}</td>);
			} else {
				cells.push(<td>&nbsp;</td>);
			}
		});
	...
	const CustomCell = (props) => {
		return (
			<td
				...
				onClick={() => alert('hai')}
			>
				{props.dataItem.items ? props.dataItem.items[0][props.field] : props.dataItem[props.field]}
			</td>
		);
	};
	const cellRender = (tdElement, cellProps) => {
		if (cellProps.rowType === 'groupHeader' && tdElement && tdElement.props.role != 'presentation') {
			const columns = [
			...
			return (
				<>
					<td {...tdElement.props} colSpan={tdElement.props.colSpan - columns.length}></td>
	...
	return (
		<React.Fragment>
			<ExcelExport data={newData} ref={_export} group={dataState.group} columns={gridColumns}></ExcelExport>
			<Grid
				dataItemKey={DATA_ITEM_KEY}
			...
			>
				<GridToolbar>
					<Button type="button" title="Export Excel" themeColor="primary" onClick={excelExport}>
						Export to Excel
					</Button>
				</GridToolbar>
				<Column
					...
					width="50px"
				/>
				<Column field="ProductName" cells={{ data: CustomCell }} title="Product Name" />
