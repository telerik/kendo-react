...
const ExpandCell = (props) => {
    ...
    const handleExpandClick = (ev) => {
        ...
        const newData = data.map((item) => ({
            ...item,
            expanded:
                item.ProductID === props.dataItem.ProductID || item.ParentID === props.dataItem.ProductID
                    ? expanded
                    : item.expanded
        }));
        setDate(newData);
        ...
        <td {...props.tdProps}>
            {props.dataItem.ParentID == 0 ? (
                <span
                    ...
                    className={'k-icon ' + (props.dataItem.expanded ? 'k-i-minus-sm' : 'k-i-plus-sm')}
                ></span>
            ) : (
                <></>
            )}
    ...
    return (
        <MyGridContext.Provider value={{ data, setDate }}>
            <Grid
                ...
                data={data}
            >
                <GridColumn field="expanded" title=" " width="50px" cells={{data: ExpandCell}} />
