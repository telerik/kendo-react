...
const App = () => {
	...
	const cancel = (dataItem) => {
		...
		const newData = data.map((item) =>
			item.ProductID === originalItem.ProductID ? originalItem : { ...item, disableEdit: false }
		);
		setData(newData);
		setDisableAddButton(false);
	};
	...
	return (
		...
		>
			<GridToolbar>
				<Button disabled={disableAddButton} title="Add new" themeColor="primary" onClick={addNew}>
					Add new
				</Button>
			</GridToolbar>
			...
			<Column cells={{ data: CommandCell }} width="200px" />
		</Grid>
	);
};
export default App;
