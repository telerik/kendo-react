...
const App = () => {
    ...
    const CustomCell = (props) => {
        ...
        if (gridRef.current) {
            ...
            if (colIndex > 0) {
                if (dataItem[columns[colIndex].field] == dataItem[columns[colIndex - 1].field]) {
                    duplicate = true;
                }
            }
            if (rowIndex > 0) {
                if (dataItem[props.field] == gridRef.current.props.data[rowIndex - 1][props.field]) {
                    duplicate = true;
                }
            }
        }
        let style = { ...props.tdProps.style };
