...
The stateful Grid performs its data operations by using the `process` method of the [`DataQuery`]({% slug overview_dataquery %}) library. To apply the changes and to save the current state of the grid data when a data operation is performed, handle the [`onDataStateChange`]({% slug api_grid_gridprops %}#toc-ondatastatechange) event.
...
You can sort, filter, or page the local data to which the KendoReact Grid is bound by using the Kendo UI Data Query component.
