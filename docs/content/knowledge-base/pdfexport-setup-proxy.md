...
<table>
    <tbody>
	    ...
	    <tr>
	    	<td>Product</td>
	    	<td>Progress® KendoReact PDFExport</td>
	    </tr>
    </tbody>
</table>
...
In your React application, render the PDFExport component settings its [forceProxy]({% slug api_pdf_pdfexportprops %}#toc-forceproxy) prop to true and [proxyURL]({% slug api_pdf_pdfexportprops %}#toc-proxyurl) to `http://localhost:4000/export`. Port 4000 is the where we will be running our Express.js project while `export` is the API route that will handle this proxy.
...
In `app.post`, we are returning `request.body.base64` that is received after exporting the PDFExport component. You now have access to the content of the PDF file on the server. After this, you can use the `XLSX` library to download the file. You can also use the Kendo [File Saver]({% slug overview_filesaver %}) package.
