...
import { saveAsPDF } from './export-to-pdf';
...
const App = () => {
    ...
    const pivotgrid = React.useRef<PivotGridHandle>(null);

    const onSaveAsPdf = React.useCallback(() => {
        const element = pivotgrid.current?.element;
        if (element) {
            saveAsPDF(element);
        }
    }, []);
