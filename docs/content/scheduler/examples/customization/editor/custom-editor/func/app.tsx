...
import { customModelFields, displayDate, rooms, sampleDataWithCustomSchema, therapists } from './data';
...
const App = () => {
    ...
    const handleDataChange = ({ created, updated, deleted }: SchedulerDataChangeEvent) => {
        setData(old => old
            .filter((item) => deleted.find(current => current.AppointmentID === item.AppointmentID) === undefined)
            .map((item) => updated.find(current => current.AppointmentID === item.AppointmentID) || item)
            .concat(created.map((item) => Object.assign({}, item, { AppointmentID: guid() }))));
    };
